...
Describe 'Toolchain command help catalog' {
	It 'covers every top-level and nested command' {
		$expected = @(
			...
			'cluster','cluster create','cluster init','cluster deinit','cluster reset','cluster list','cluster status','cluster kubeconfig','cluster use','cluster current','cluster delete',
			'k9s','doctor','help'
		)
	...
	It 'returns an overview with command-scoped help instructions' {
		...
		(Invoke-ToolchainHelp -CommandPath @('package','remove')) | Should -Match '-Confirm'
	}
