...
- Accept `log_level`, `log_format`, `package.create.skip_sbom`, and nested `package.create` and `package.deploy` settings in `toolchain-config.yaml`, including creation templates, deployment variables, component selection, and bundled deploy-time Helm values.
